...
def test_score_session_returns_all():
    ...
    scorer_names = {s.scorer for s in scores}
    assert "implicit.correction_free_rate" in scorer_names
    assert "implicit.completion" in scorer_names
    assert "implicit.correction_density" not in scorer_names
    assert "implicit.abandonment" not in scorer_names
