...
def test_schema_v7_resets_disposable_database_on_version_mismatch(tmp_path):
    ...
    assert RUN_DB_SCHEMA_VERSION == 7
...
def test_schema_v7_resets_collided_v5_database_missing_judging_artifacts(tmp_path):
    path = tmp_path / "runs.db"
    connection = sqlite3.connect(path)
...
def test_schema_v7_resets_schema_v6_rows_with_stale_effective_config(tmp_path):
    path = tmp_path / "runs.db"
    connection = sqlite3.connect(path)
