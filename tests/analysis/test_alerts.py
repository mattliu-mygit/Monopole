...
def test_active_roundtrip(tmp_path):
    p = str(tmp_path / "active.json")
    assert alerts.load_active(p) == set()  # missing file → empty
    alerts.save_active(p, {"a", "b"})
    assert alerts.load_active(p) == {"a", "b"}


def test_load_active_none_path():
    assert alerts.load_active(None) == set()


def test_load_active_non_list_json_is_empty(tmp_path):
    p = tmp_path / "active.json"
    p.write_text('{"not": "a list"}')  # hand-corrupted into an object
    assert alerts.load_active(str(p)) == set()


def test_load_active_rejects_non_string_list_entries_without_crashing(tmp_path):
    p = tmp_path / "active.json"
    p.write_text('["valid", {"not": "hashable"}]')

    assert alerts.load_active(str(p)) == set()


def test_save_active_creates_missing_parent_dir(tmp_path):
    p = str(tmp_path / "nested" / "dir" / "active.json")
    alerts.save_active(p, {"a"})  # parent dirs don't exist yet
    assert alerts.load_active(p) == {"a"}


def test_save_active_bad_path_does_not_raise(tmp_path):
    # a path whose parent is a file, not a dir → OSError, must be swallowed
    afile = tmp_path / "afile"
    afile.write_text("x")
    alerts.save_active(str(afile / "active.json"), {"a"})  # must not raise
