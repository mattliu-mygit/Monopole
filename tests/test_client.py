...
def test_children_query_requests_details(client):
    """The children query must set include_details to pull tool args/results."""
    body = client._build_children_query("tr-1")
...
def test_hydrate_children_http(mock_client):
    respx.post(f"{TRACE_BASE}/agents/spans/query").respond(
        json={"spans": [_fake_child_span(span_id="sp-tool1")]},
    )
